...
std::string SystemTrayIcon::Impl::make_tooltip_text() const
{
    if (auto* const session = core_->get_session(); session != nullptr)
    {
        return fmt::format(
            ...
            fmt::arg("upload_speed", Speed{ tr_sessionGetRawSpeed_KBps(session, tr_direction::Up), Speed::Units::KByps }.to_string()),
            fmt::arg("download_speed", Speed{ tr_sessionGetRawSpeed_KBps(session, tr_direction::Down), Speed::Units::KByps }.to_string()));
    }
